...
角色设定系统的目的不是写百科，而是保证翻译中角色语气一致、称呼关系一致、口癖一致、敬语等级一致，并防止润色阶段把所有人润成同一种声音。
...
角色条目可以使用 `candidate`、`approved`、`needs_review`、`conflict`、`locked` 等状态。角色姓名和称呼一旦 locked，初翻、润色和审核流程不得自动覆盖。

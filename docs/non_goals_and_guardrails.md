...
| 任务类型 | 后置到 |
| --- | --- |
| UI 视觉抛光（P2 级） | S14（FS-065…FS-068） |
| 阅读体验 / 信息密度优化 | S14 |
...
- 修改 `docs/product_final_state_spec.md`（须按其 §27 流程说明五项理由）；
- 对外发布任何译文。
...
- 自动覆盖原文、baseline、production_candidate、人工校对译文；
...
真实译文（output_draft/ output_refined/ output_final/ draft_full_baseline/
  refined_full_candidate/ production_candidate/ 正文）
...
- 覆盖任何输出；
- 导入用户修改稿并执行同步；
- 存在 blocking issue 时导出 production package。
...
满足以上即可按轮次清单继续，无需用户逐轮确认（里程碑闸门除外：baseline lock、production_candidate、human_approved_final）。
...
- 译文产物只能写入规定输出目录；baseline 与 production_candidate 生成后写保护；
- 报告与文档中引用正文时只允许小段脱敏示例或纯统计；
- 上下文注入遵守规格 §22：仅当前 batch 相关内容，禁止全书回溯。

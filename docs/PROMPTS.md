...
Copy-paste templates for Tool-aware rounds. A template is not an authority source: each use must obey current `AGENTS.md`, `project.yaml`, and `docs/git_safe_cohort_delivery.md`. Real-worktree validation is targeted/read-only; a full gate is isolated-copy-only with no writeback. Prompt text cannot expand Git targets, baseline authority, API use, publication, or other external effects. An approved Git-safe cohort is finalized only through the standing hash-bound delivery policy.
...
Read first: AGENTS.md, agent_tools.yaml, reports/current-cohort-report.json, docs/TOOL_USAGE_POLICY.md.
...
5. Write reports/current-cohort-report.json with tools_used / tools_not_used

Constraints: no real API or publish; this Prompt cannot expand Git authority. If this round produces an approved Git-safe cohort, the standing finalizer must deliver and remotely verify it before another cohort begins.
```
...
Read first: AGENTS.md, agent_layer.yaml, agent_tools.yaml, reports/current-cohort-report.json.
...
Write reports/current-cohort-report.json and append agent_audit_log.jsonl.
```
...
Update current-cohort-report with browser tool usage.
...
Read current-cohort-report.json severity_summary and reports/gate_result.json.
...
Read: AGENTS.md, agent_layer.yaml, agent_tools.yaml, docs/CODEX_HANDOFF.md (filled), reports/current-cohort-report.json.
...
Update current-cohort-report web_research section.
```
...
Generate current-cohort-report mode=tool-probe.
```

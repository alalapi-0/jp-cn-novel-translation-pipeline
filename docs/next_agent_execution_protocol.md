...
④ 状态探针：python3 scripts/agent_gate.py --json
            python3 scripts/check_orphan_workers.py --json
            python3 scripts/local_scheduler_status.py --json（存在后）
...
⑪ Git：git status --short && git diff --stat && git diff --check；
   只 add 本轮相关文件（禁止 git add .）；确认无密钥 / 正文后 commit
⑫ Push：仅用户已授权时执行；失败一次即记录原因，不反复重试
⑬ 输出下一轮建议（轮次号 + 一句话目标）
```

...
from write_agent_report import (  # noqa: E402
    append_audit_log,
    build_template,
    merge_report,
...
def main() -> int:
    ...
    current_cohort_report = merge_report(
        build_template(
            round_id="governance-final-consistency-cleanup",
    ...
    valid, errors = validate_report_dict(current_cohort_report)
    if not valid:
        raise RuntimeError("current cohort report is invalid: " + "; ".join(errors))
    write_report_file(current_cohort_report)
    append_audit_log(
        round_id=current_cohort_report["round_id"],
        agent=current_cohort_report["agent"],
        agent_surface=current_cohort_report["agent_surface"],
        gate_status=current_cohort_report["gate_status"],
        summary="Consistency product finalized; Git-safe cohort remains pending remote SHA verification.",
        timestamp=current_cohort_report["timestamp"],
    )

    result = {
        "product_status": "consistency_completed",
        "git_cohort_status": "candidate_ready_for_delivery",
    ...
    if args.json:
        ...
        print(
            "finalize_consistency_run: product completed; Git-safe cohort pending "
